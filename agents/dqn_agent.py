...
class DQN_agent:
    ...
    def act(self, state, train):
        ...
        if np.random.rand() > self.epsilon or not train:
            with torch.no_grad():
                return self.model(state.reshape(1,len(state))).reshape(-1).data.max(0)[1].detach().cpu()
        else:
    ...
    def train(self):
        ...
        current_q_values = self.model(batch_state.reshape(self.batch_size,len(batch_state[0]))).gather(1,batch_action)
        with torch.no_grad():
            max_next_q_values = self.model(batch_next_state.reshape(self.batch_size,len(batch_state[0]))).detach().max(1)[0]
